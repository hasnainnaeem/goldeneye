...
class goldeneye(core.fault_injection):
    ...
    @staticmethod
    def transform_numsys(tensor, num_sys, quant, signed, qsigned, bits, R_quantize_signed=2, R_quantize_unsigned=1, precision="FP32"):
        # for simulated number system
        (num_sys_config, num_sys_name) = num_sys

        if quant is False:
            tensor[:] = num_sys_config.convert_numsys_tensor(tensor, meta_inj=False)
    ...
    @staticmethod
    def quantizeSigned_INPLACE(precision, X, B, R=1.0):
        # normalize first
    ...
    @staticmethod
    def quantizeUnsigned(precision, X, B, R=2.0):
        getMode = precision
        S = 2.0 / R
    ...
    def apply_goldeneye_transformation(self, module, input, output):
        ...
        if self.quant is False:
            ...
            # quantize (scale and quant NumSys)
            if self.qsigned:
                output[:] = goldeneye.quantizeSigned_INPLACE(self.precision, output, self.bits, R=range_max)
            else:
                output[:] = goldeneye.quantizeUnsigned(self.precision, output, self.bits, R=range_max)

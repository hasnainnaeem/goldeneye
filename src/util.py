...
def parseConfig():
    ...
    config["loadDatasetPercentage"] = int(
        dataset_info["loadDatasetPercentage"])
    ...
    if dataset_info["imgRangesGiven"].lower() != "no" and dataset_info["imgRangesGiven"].lower() != "false":
        config["imgRangesGiven"] = True
        config["imgRangeMax"] = float(dataset_info["imgRangeMax"])
        config["imgRangeMin"] = float(dataset_info["imgRangeMin"])
    else:
        config["doNormalize"] = False
        config["imgRangeMax"] = None
        config["imgRangeMin"] = None
...
def getNetwork(networkName, DATASET):
    ...
    if "custom" in DATASET.lower():  # covers custom cifar10, cifar100 datasets
        if networkName == "resnet18":
            ...
            MODEL = cifar10_nn.baseline(
                pretrained=True, output_size=getNumClasses(DATASET))
    ...
    if DATASET.lower() == 'imagenet':
        ...
        # transformers
            ...
            MODEL = torch.hub.load(
                FB_repo, 'deit_base_patch16_224', pretrained=True)
        elif networkName == "deit_tiny":
            MODEL = torch.hub.load(
                FB_repo, 'deit_tiny_patch16_224', pretrained=True)
...
def load_id_custom_dataset(DATASET, BATCH_SIZE, norm_mean, norm_std, dataset_path, dataset_percentage=100, workers=0,
    ...
    num_sys_config = getNumSysName(getFormat(),
                                    bits=bitwidth_fp,
                                    radix_up=exp_bits,
    ...
    if random:
        if replacement:
            if single == False:
                ...
                custom_sampler = get_custom_sampler_single(
                    good_images, singleIndex, total_data)

        else:
            custom_sampler = get_custom_sampler_no_replacement(
                good_images, total_data)
    else:
...
def load_file(file_name, compress=True):
    f = bz2.BZ2File(file_name + '.p.bz2',
                    "rb") if compress else open(file_name.strip('.p.bz2'), "rb")
